...
if __name__ == "__main__":
  ...
  trends = trends.union(temp)
  temp = df.groupby('trans_dt','trans_key').agg(collect_list('trans_key').getItem(0).alias('trans_key_temp'))
  ...
  temp = temp.withColumn('measure', lit('total distinct collectors'))
  trends = trends.union(temp)
  temp = df.groupby('trans_dt','collector_key').agg(collect_list('collector_key').getItem(0).alias('collector_key_temp'))
